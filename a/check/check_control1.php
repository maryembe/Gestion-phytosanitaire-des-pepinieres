...
//vérifier si les variables POST 'date_ctrl1', 'conform'  sont entres 
...
/*vérifier si les champs 'date_ctrl1' et 'conform' ne sont pas vides. Si l'un des deux est vide,
...
$dbname = "database";
       ...
       /* if ($previousConform === "nonConforme" && $conform === "conforme") {
            ...
            $sql = "INSERT INTO ctrl1 (date_ctrl1, conform , id_ctrl_doc ) VALUES (:date_ctrl1, :conform, :id_ctrl_doc)ON DUPLICATE KEY UPDATE
            date_ctrl1 = :date_ctrl1, conform = :conform";
            $stmt = $pdo->prepare($sql);
            $stmt->execute(['date_ctrl1' => $date_ctrl1, 'conform' => $conform , 'id_ctrl_doc' => $id_ctrl_doc ]);
            // Recuperation de id_ctrl1
            $sql1 = "SELECT * FROM ctrl1 WHERE id_ctrl1 = (SELECT MAX(id_ctrl1) FROM ctrl1 WHERE id_ctrl_doc = :id_ctrl_doc)";
          ...
          /*  $sql10 = "SELECT * FROM ctrl_doc WHERE id_ctrl_doc = :id_ctrl_doc";
            ...
            $sql100 = "INSERT INTO courrier (id_ctrl1, Result_ctr1  ) VALUES (:id_ctrl1 ,:Result_ctr1)ON DUPLICATE KEY UPDATE
            id_ctrl1 = :id_ctrl1, Result_ctr1 = :Result_ctr1";
            $stmt100 = $pdo->prepare($sql100);
            $stmt100->execute(['id_ctrl1' => $id_ctrl1, 'Result_ctr1' => $Result_ctr1  ]);
