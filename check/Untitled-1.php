...
/*vérifier si les champs 'date_ctrl_doc' et 'conform' ne sont pas vides. Si l'un des deux est vide,
 l'utilisateur est redirigé vers la page de Control avec un message d'erreur approprié.*/
	if (empty($date_ctrl1)) {
		header("Location: ../php/Control1.php?error=Date Control is Required&id_ctrl_doc=" . $_SESSION['id_ctrl_doc']);
	}else if (empty($conform)) {
		header("Location: ../php/Control1.php?error=Result is Required&id_ctrl_doc=" . $_SESSION['id_ctrl_doc']);
	}else {
...
$dbname = "database";
       ...
       /* if ($previousConform === "nonConforme" && $conform === "conforme") {
            ...
            $sql1 = "SELECT id_ctrl1 FROM ctrl1 WHERE id_ctrl_doc = :id_ctrl_doc";
...
$sql2 = "SELECT *
        ...
        $result2 = mysqli_query($conn, $sql2);
        	...
        	$row2 = mysqli_fetch_assoc($result2);
        		...
        		header("Location: ../php/Control1.php?id_ctrl1=" . $_SESSION['id_ctrl1']. "&Etat=" . $_SESSION['Etat']);
...
}}}else {header("Location: ../php/Control1.php?id_ctrl_doc=" . $_SESSION['id_ctrl_doc']. "&Etat=" . $_SESSION['Etat']);
