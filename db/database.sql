...
CREATE TABLE `database`.`declaration` (
    ...
    `Etat` enum('Non prise en charge ','Control documentaire non conforme ',"Control documentaire conforme","Control Physique 1 non conforme",,"Control Physique 1 conforme","Control Physique 2 non conforme","Control Physique 2 non conforme") NOT NULL ,
    id_pep VARCHAR(50) REFERENCES pep(username),
...
___________________________________________
Courrier
CREATE TABLE `database`.`courrier` (`Num` INT NOT NULL ,`date_ctrl_doc` DATE NOT NULL REFERENCES ctrl_doc(date_ctrl_doc) ,id_ctrl_doc INT REFERENCES ctrl_doc(id_ctrl_doc),"Result_ctrl_doc" VARCHAR(150)  ,id_ctrl1 INT REFERENCES ctrl1(id_ctrl1),`date_ctrl1` DATE NOT NULL REFERENCES ctrl1(date_ctrl1),"Result_ctr1" VARCHAR(150)   ,PRIMARY KEY (`Num`))
________________________________________________
acp
